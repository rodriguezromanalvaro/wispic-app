-- Fixes for Supabase Advisor (Performance) - Batch 4
-- Date: 2025-10-16
...
BEGIN
  FOR r IN
    ...
    WHERE (n.nspname, c.relname, pol.polname) IN (
      ('public','user_statistics','Users can view their own statistics'),
      ('public','user_statistics','System can update user statistics'),
      ('public','achievement_templates','Achievement templates are viewable by all authenticated users'),
      ('public','user_achievements','Users can view their own achievements'),
      ('public','user_achievements','System can update user achievements'),
      ('public','user_levels','Users can view their own levels'),
      ('public','user_levels','System can update user levels')
    )
  LOOP
    -- roles
