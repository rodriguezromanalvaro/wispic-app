...
CREATE TABLE public.event_series (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  tzid text NOT NULL DEFAULT 'Europe/Madrid'::text,
  start_date date,
  end_date date,
  start_time time without time zone,
  end_time time without time zone,
  days_of_week ARRAY DEFAULT '{}'::smallint[],
  auto_roll boolean NOT NULL DEFAULT true,
  roll_ahead_weeks integer NOT NULL DEFAULT 12,
  is_free_default boolean,
  price_cents_default integer,
  currency_default text DEFAULT 'EUR'::text,
  ticket_url_default text,
