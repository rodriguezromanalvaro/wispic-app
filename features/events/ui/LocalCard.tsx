...
export const LocalCard = memo(function LocalCard(props: LocalCardProps) {
  const {
    ...
    togglingIds, onToggleExpand, onToggleGoing, onOpenOccurrence, onOpenAttendees, onSeeAllOccurrences,
    priceLabel
  } = props;
  ...
  return (
    <GlassCard elevationLevel={1} interactive tint={sponsored ? 'primary':'none'} style={{ margin:16, marginTop:6 }} padding={14}>
      {/* Header: title + attendees (removed primary series-level go button) */}
  <XStack ai="flex-start" mb={6} style={{ flexDirection:'row', alignItems:'flex-start', marginBottom:6 }}>
  ...
  <XStack ai="center" gap={8} style={{ flexDirection:'row', alignItems:'center', gap:8 }}>
        ...
        {!expanded && (
          <XStack ai="center" style={{ marginTop:8, alignSelf:'flex-start', backgroundColor: theme.colors.card, borderWidth:1, borderColor: theme.colors.primary + '55', paddingHorizontal:10, paddingVertical:4, borderRadius:14, flexDirection:'row' }}>
      ...
      <Animated.View style={[{ marginTop: expanded ? 10 : 0 }, occStyle]}> 
        {expanded && (
        <YStack style={{ borderTopWidth:1, borderTopColor: theme.colors.border, paddingTop:8, gap:6 }}>
          {occurrences.map(o => {
