...
interface Props { visible: boolean; onClose: () => void; profileId: string; initial: { interested_in: string[]; show_orientation: boolean }; }

export const EditOrientationSheet: React.FC<Props> = ({ visible, onClose, profileId, initial }) => {
  const { t } = useTranslation();
  const mutations = useProfileMutations(profileId);
  const [selected, setSelected] = useState<string[]>(initial.interested_in || []);
  const [showing, setShowing] = useState<boolean>(initial.show_orientation);
  const toggle = (k: string) => setSelected(cur => cur.includes(k) ? cur.filter(x => x !== k) : [...cur, k]);
  const save = () => {
    // Reuse basics mutation if it supports; else create a custom orientation update.
    mutations.updateBasics.mutate({ interested_in: selected });
    mutations.updateVisibility.mutate({ show_orientation: showing });
    onClose();
  };
  return (
      ...
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          <View style={styles.headerRow}>
            <Text style={styles.title}>{t('profile.orientation.editTitle','Editar orientación')}</Text>
              ...
              {ORIENT_OPTIONS.map(o => {
                ...
                return (
                  <TouchableOpacity key={o.key} style={[styles.option, active && styles.optionActive]} onPress={() => toggle(o.key)}>
                    <Text style={[styles.optionText, active && styles.optionTextActive]}>{o.icon} {t(o.labelKey, o.key)}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
...
export default EditOrientationSheet;