// features/profile/ui/ProfilePreviewPane.tsx
...
import { ProfileDraft } from 'features/profile/model';
import { getGenderLabel, toOrientationLabels } from 'features/profile/model/mappings';
...
export function ProfilePreviewPane({ draft, showOrientation = true }: { draft: ProfileDraft; showOrientation?: boolean }) {
  ...
  return (
    <TgYStack p="$2" br="$lg" bg="$surfaceMuted" borderColor="$border" style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        {draft.prompts?.length ? (
          <TgYStack mt="$2">
            {draft.prompts.map((p: { key: string; answers?: string[] }) => (
              <P key={p.key} style={styles.promptLine}>{t(`complete.prompt.${p.key}.title`, p.key)}: {(p.answers||[]).map((ans: string) => t(`complete.prompt.${p.key}.answers.${ans}`, ans)).join(' • ')}</P>
            ))}
          </TgYStack>
        ) : null}
      </ScrollView>
