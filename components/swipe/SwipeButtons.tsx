...
function Ring({ size, color, children }: { size: number; color: string; children?: React.ReactNode }){
  const radius = size / 2
  return (
    <View style={{ width: size, height: size, borderRadius: radius, alignItems:'center', justifyContent:'center' }}>
      <LinearGradient
        colors={[color + '55', color + '00']}
        start={{ x: 0.3, y: -0.2 }} end={{ x: 1, y: 1 }}
        style={{ position:'absolute', top:-6, right:-6, bottom:-6, left:-6, borderRadius: radius + 10 }}
      />
      {children}
    </View>
  )
...
export const SwipeButtons: React.FC<Props> = ({ onPass, onLike, onSuperLike, remaining }) => {
  return (
